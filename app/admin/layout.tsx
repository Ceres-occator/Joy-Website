...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-zinc-100 text-zinc-900 flex flex-col lg:grid lg:grid-cols-[280px_1fr]">
      ...
      {/* 💻 DESKTOP SIDEBAR */}
      <aside className="hidden lg:flex flex-col justify-between rounded-br-[3rem] bg-zinc-950 px-6 py-8 text-white shadow-2xl h-screen sticky top-0">
        <div className="space-y-10">
          ...
          <nav className="space-y-2 text-sm font-medium">
            {navLinks.map((link) => (
              ...
              >
                <span className="mr-2.5">{link.icon}</span> {link.label}
              </Link>
            ))}
          </nav>
        </div>
